...
void main() {
    ...
    vec3 textureLoc = vec3(vec2(
        int(texturePos.x) & 15,
        int(texturePos.y) & 15),
        0
    );
    vec3 deltaPos = (worldPos + textureLoc * .0175);
    //worldPos.y += GameTime * ChromaConfig.b * GAME_TIME_SCALE;
    float chroma = cnoise(deltaPos) * .5 + .5;// * ChromaConfig.g;
