...
public class ArtificerTableScreen extends AbstractContainerScreen<ArtificerTableMenu> {
    ...
    public ArtificerTableScreen(ArtificerTableMenu menu, Inventory inventory, Component component) {
    ...
    protected void renderBg(@NotNull GuiGraphics graphics, float pPartialTick, int pMouseX, int pMouseY) {
        ...
        boolean[] unlocked = this.menu.getUnlocked();
        int xStart = 25 - (ArtificerTableMenu.MAX_GEMSTONE_SLOTS - 5) * 13;
        for (int i = 0; i < ArtificerTableMenu.MAX_GEMSTONE_SLOTS; i++) {
            if (!unlocked[i]) continue;
            UsefulTextures.renderSlotBackground(graphics, this.leftPos + xStart + 27 * i, this.topPos + 53);
        }
